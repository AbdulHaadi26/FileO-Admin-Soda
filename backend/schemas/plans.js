module.exports = {
    ...
    getAllPlanCount: async (_id, collection) => {
        try {
            const doc = await collection.find().filter({ postedby: _id }).count();
    ...
    getAllPlanLimit: async (_id, limit, collection) => {
        try {
            let plans = [];
            let skipInNumber = Number(limit);
            skipInNumber = skipInNumber * 12;
            const doc = await collection.find().filter({ $query: { postedby: _id }, $orderby: { created: -1 } }).skip(skipInNumber).limit(12).getDocuments();
    ...
    getAllPlanQueryCount: async (_id, string, collection) => {
        try {
            const doc = await collection.find().filter({ postedby: _id, name: { $upper: { $regex: `.*${string.toUpperCase()}.*` } } }).count();
            if (doc) return doc.count;
            return 0;
        } catch (e) {
            throw new Error(e.message);
    ...
    getAllPlanQueryLimit: async (_id, limit, string, collection) => {
        try {
            let plans = [];
            let skipInNumber = Number(limit);
            skipInNumber = skipInNumber * 12;
            const doc = await collection.find().filter({ $query: { postedby: _id, name: { $upper: { $regex: `.*${string.toUpperCase()}.*` } } }, $orderby: { created: -1 } }).skip(skipInNumber).limit(12).getDocuments();
            if (doc) doc.map(document => {
                let tempDoc = document.getContent();
                tempDoc._id = document.key;
                plans.push(tempDoc);
