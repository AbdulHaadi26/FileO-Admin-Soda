...
module.exports = {
    ...
    findActivePackages: async (size, collection) => {
        try {
            ...
            pkgSize = pkgSize + 1;
            const docs = await collection.find().filter({ active: true, size: { $gte: pkgSize }, flag: 'P' }).getDocuments();
            let contents = [];
    ...
    findLowerPackages: async (lower, upper, collection) => {
        try {
            ...
            let docs = await collection.find().filter({ active: true, size: { $lt: high, $gt: low }, flag: 'P' }).count();
