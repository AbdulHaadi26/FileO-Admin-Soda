module.exports = {
        ...
        try {
            ...
            if (type === 'Name') {
                ...
                if (due === 'Due') {
                    ...
                    doc = await collection.find().filter({ postedby: _id, time_due: { $lte: dateR, $gte: date } }).count();
        ...
        try {
            ...
            if (type === 'Name') {
                ...
                if (due === 'Due') {
                    ...
                    dateR.setHours(0, 0, 0, 0);
                    dateR.addHours(24 * (7 - date.getDay()));
                    doc = await collection.find().filter({ $query: { postedby: _id, time_due: { $lte: dateR, $gte: date } }, $orderby: { created: -1 } }).skip(skipInNumber).limit(12).getDocuments();
                } else if (due === 'Due Next Week') {
                    let dateR = new Date(Date.now()), dateL = new Date(Date.now());
                    dateR.setHours(0, 0, 0, 0);
                    dateL.setHours(0, 0, 0, 0);
        ...
        try {
            ...
            if (type === 'Name') {
                ...
                if (due === 'Due') {
                    ...
                    dateR.setHours(0, 0, 0, 0);
                    dateR.addHours(24 * (7 - date.getDay()));
                    doc = await collection.find().filter({ postedby: _id, time_due: { $lte: dateR, $gte: date }, name: { $upper: { $regex: `.*${string.toUpperCase()}.*` } } }).count();
                } else if (due === 'Due Next Week') {
                    let dateR = new Date(Date.now()), dateL = new Date(Date.now());
                    dateR.setHours(0, 0, 0, 0);
                    dateL.setHours(0, 0, 0, 0);
        ...
        try {
            ...
            if (type === 'Name') {
                ...
                if (due === 'Due') {
                    ...
                    dateR.setHours(0, 0, 0, 0);
                    dateR.addHours(24 * (7 - date.getDay()));
                    doc = await collection.find().filter({ $query: { postedby: _id, time_due: { $lte: dateR, $gte: date }, name: { $upper: { $regex: `.*${string.toUpperCase()}.*` } } }, $orderby: { created: -1 } }).skip(skipInNumber).limit(12).getDocuments();
                } else if (due === 'Due Next Week') {
                    let dateR = new Date(Date.now()), dateL = new Date(Date.now());
                    dateR.setHours(0, 0, 0, 0);
                    dateL.setHours(0, 0, 0, 0);
    ...
    findPlanByName: async (value, _id, collection) => {
        try {
            const doc = await collection.find().fetchArraySize(0).filter({ name: value, postedby: _id }).getOne();
            if (doc) return true;
            return false;
        } catch (e) {
            throw new Error(e.message);
    ...
    getAllPlanCountDash: async (_id, collection) => {
        try {
            doc = await collection.find().filter({ postedby: _id }).count();
            if (doc) return doc.count;
            return 0;
