...
const {
    ...
    findPlanByName,
    updatePlanDetails
} = require('../../../schemas/personal/schemas/plans');
...
router.put('/register', JWT, async (req, res) => {
    ...
    try {
        ...
        let pL = await findPlanByName(name, _id, collectionPlan);

        if (pL) throw new Error('Plan with this name already exists');

        let data = {
            name, postedby: _id, started: date,
...
router.post('/updateDetails', JWT, async (req, res) => {
    ...
    try {
        ...
        if (pL.name !== name) isTrue = await findPlanByName(name, req.token._id, collectionPlan);
