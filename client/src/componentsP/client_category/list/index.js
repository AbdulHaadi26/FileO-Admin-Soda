...
import {
    ...
    updateCatC,
    deleteCat
} from '../../../redux/actions/personal/clientCategoryAction';
...
const ListCat = ({
    fetchCombined, _id, handleT, tabNav, setTN, pId, fileData, string, handleS, type, isList, handleISL, registerCat, updateCatC,
    ...
    return <div className="col-11 c-d-w p-0">
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'row', flexWrap: 'wrap', marginTop: '12px' }}>
            <DragDropContext onDragEnd={handleOnDragEnd}>
                {list && list.length > 0 &&
                    <FolderList uId={_id} list={list} ord={ordF} isList={isList}
                        setMUpt={setMUpt} setMD={setMD} setCID={setCID} disabled={disabled} />}
                {listF && listF.length > 0 &&
                    <FileList list={listF} isList={isList} ord={ordF} uId={pId} disabled={disabled}
        ...
        {sM && <Suspense fallback={<></>}><ModalLink _id={_id} showModal={handleModal} /></Suspense>}
        {catId && <Suspense fallback={<></>}><ModalLinkF catId={catId} _id={_id} showModal={e => setCID(false)} /></Suspense>}
