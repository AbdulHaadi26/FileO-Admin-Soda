...
import { downloadFile } from '../../../redux/actions/personal/clientFilesAction';
                ...
                <Draggable key={File._id} draggableId={`File-${File._id}`} index={k}>
                    {(provided) => (
                        <div className="LI" style={{ borderRadius: '4px', position: 'relative' }} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            ...
                            <Link style={{ textDecoration: 'none', marginLeft: '12px', fontSize: '14px', fontWeight: '400' }} className="f-n mr-auto" to={`/personal/user/${uId}/clients/file/${File._id}`}>{File.name}  {File.updated && <span className="static" style={{ color: 'red' }}>(Updated)</span>}</Link>
                    ...
                    {(provided) => (
                        <div className="col-12 mFWS" key={File._id} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            ...
                            <img src={returnType(File.type)} style={{ cursor: 'pointer' }}
                                onClick={e => history.push(`/personal/user/${uId}/clients/file/${File._id}`)} alt="compnay" />
