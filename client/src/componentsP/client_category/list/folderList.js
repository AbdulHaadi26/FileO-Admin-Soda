    ...
    return isList ? listT.map((Cat, k) => <Droppable key={Cat._id} droppableId={`drop-Folder-${Cat._id}`}>
        {(provided) => (
            <div className="LI" {...provided.droppableProps} ref={provided.innerRef} key={Cat._id}>
                ...
                <Link style={{ textDecoration: 'none', marginLeft: '12px', wordBreak: 'break-all' }}
                    to={`/personal/user/${uId}/clients/files/${Cat._id}/list`} className="mr-auto">{Cat.name} {Cat.updated && <span style={{ color: 'red', fontSize: '12px' }}>(Updated)</span>}</Link>
        ...
        {(provided) => (
            <div className="col-lg-2 col-4 mFWS" {...provided.droppableProps} ref={provided.innerRef}>
                ...
                <img src={Folder} alt="Folder" style={{ cursor: 'pointer' }} onClick={e => history.push(`/personal/user/${uId}/clients/files/${Cat._id}/list`)} />
                <Link to={`/personal/user/${uId}/clients/files/${Cat._id}/list`}
