...
import { clearCut, uploadType } from '../../redux/actions/personal/userFilesActions';
const mT = { marginTop: '16px', padding: '6px 12px' };

const UploadFile = ({ file, clearCut, uploadType, handleVer, handleNew, note, getList }) => {
    return file && file.error === 'User File' ? <Modal handleModal={e => {
        clearCut();
        getList();
    }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', padding: '6px 12px', marginTop: '12px', marginBottom: '12px' }}>
            <button className="btn btn-primary" type="button" style={{ fontSize: '14px', fontWeight: '600', padding: '6px 24px' }} onClick={async e => {
                if (file.type !== 1) {
            ...
            {!note && <button className="btn btn-primary" type="submit" style={{ marginLeft: '12px', fontSize: '14px', fontWeight: '600', padding: '6px 24px' }} onClick={async e => {
                if (file.type !== 1) {
