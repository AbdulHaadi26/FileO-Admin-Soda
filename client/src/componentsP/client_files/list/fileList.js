...
import { deleteFiles, cutFiles, copyFiles, downloadFile } from '../../../redux/actions/personal/clientFilesAction';
...
const List = ({ id, uId, list, sL, onsetSL, isList, ord, sv, disabled, downloadFile, setMDF, setEF }) => {
    ...
    const renderList = () => listT.map((File, k) => isList ? <div className="LI" key={File._id}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', marginTop: '6px', marginLeft: '6px' }} className="mr-auto">
            <Link style={{ textDecoration: 'none', wordBreak: 'break-all', fontSize: '14px', fontWeight: '400' }} to={`/personal/user/${uId}/clients/file/${File._id}`}>{File.name} {File.updated && <span style={{ color: 'red', fontSize: '12px', fontWeight: '400' }}>(Updated)</span>}</Link>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end', marginLeft: 'auto' }}>
            ...
            <img style={{ cursor: 'pointer' }} src={returnType(File.type)}
                onClick={e => history.push(`/personal/user/${uId}/clients/file/${File._id}`)} alt="company" />
            <Link style={{ textDecoration: 'none', wordBreak: 'break-all' }} to={`/personal/user/${uId}/clients/file/${File._id}`} className="f-n">{File.name.length > 35 ? `${File.name.substr(0, 35)}...` : File.name}</Link>
