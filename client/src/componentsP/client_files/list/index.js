...
import { copyFiles, cutFiles, deleteFile, deleteFiles, fetchFile, getFileDetailsM } from '../../../redux/actions/personal/clientFilesAction';
...
const List = ({
    fetchFile, _id, catId, fileData, isSuc, string,
    ...
    return <div className="col-11 f-w p-0">
        ...
        {isSuc && list && list.length > 0 ?
            <FileList isList={isList} sL={sL} onsetSL={handleSL} catId={catId} list={list} disabled={disabled}
                ord={ord} count={count} uId={_id} onFetch={fetch} setMDF={setMDF} setEF={val => { setEF(val); getDetails(val); }} />
            : <div> <h6 className="f-n" style={eS}>No file found</h6> </div>}
        {sM && <Suspense fallback={<></>}><ModalLink _id={_id} catId={catId} showModal={handleModal} /></Suspense>}
