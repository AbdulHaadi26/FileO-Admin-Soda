...
const Modal = ({ percent, fileData, setting, onFinish }) => {
    const [i, setI] = useState(0), [text, setT] = useState('Please wait while we upload your file to File-O'),
        [fName, setFN] = useState(''), [fType, setFT] = useState(''), [fSize, setFS] = useState(0);
    ...
    useEffect(() => {
        ...
        if (fileData && fileData.name && fileData.type && fileData.size) {
            let size = (fileData.size * 1000);
            setFN(`${fileData.name}.${fileData.type[0]}`);
            setFS(size);
                    ...
                    <div className="fl-sw">
                        ...
                        <div className="fl-sb-w">
                            ...
                            <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                                <h6 className="fl-n">({`${fSize.toFixed(2)}`}MB)</h6>
                                <h6 className="fl-n">{percent}%</h6>
                            </div>
