...
const Modal = ({
    onhandleModal, data_uploaded, combined_plan, available, percent, packages, getPackagesP, org, submitData, active_plan, ModalProcess
}) => {
    ...
    useEffect(() => {
        combined_plan && getPackagesP({ size: Math.floor(Math.round(combined_plan - 1)) });
    }, [combined_plan, getPackagesP]);
    ...
    const submitForm = e => {
        ...
        let data = {
            ...
            price: (pkg.size - combined_plan) * 6
        };
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <form className="col-lg-6" style={{ display: 'flex', flexDirection: 'column', margin: '18px 0px', alignItems: 'center', border: '2px solid #dfe6e9', borderRadius: '4px', padding: '24px 12px' }} onSubmit={submitForm}>
                ...
                <h6 style={{ fontSize: '16px', color: 'green', fontWeight: '500', width: '90%' }}>
                    PKR {pkg ? ((Number(pkg.size) - Number(combined_plan))  * 6) : 0}
                </h6>
