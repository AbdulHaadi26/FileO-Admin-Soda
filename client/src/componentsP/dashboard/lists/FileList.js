...
export default ({ fileList }) => {
    ...
    const renderUrl = Item => {
        switch (Item.location) {
            case 'Client Files': return `/personal/user/${Item.postedFor}/clients/file/${Item._id}`;
            default: return `/personal/myspace/user/${Item.postedby}/file/${Item._id}`
        }
    }

    return list && list.length > 0 ? list.map(Item => <div className="LIn" onClick={e => history.push(renderUrl(Item))} key={Item._id} style={{ cursor:'pointer', padding: '4px 6px', borderRadius: '4px', position: 'relative' }}>
