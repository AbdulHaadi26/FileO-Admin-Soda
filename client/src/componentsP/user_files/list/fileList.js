...
const List = ({  uId, list, ord, sv, handleSel, sL, isList, downloadFile, setLink, setDel, setF, setEF, setVerN, setCopy, setMove, disabled }) => {
                    ...
                    {(provided) => (
                        <div className="LI" style={{ borderRadius: '4px', position: 'relative' }} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            ...
                            <div className="mr-auto" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                <Link style={{ textDecoration: 'none', marginLeft: '12px', wordBreak: 'break-all' }} to={`/personal/myspace/user/${uId}/file/${File._id}`}>{File.name} {File.updated && <span className="static" style={{ color: 'red' }}>(Updated)</span>}</Link>
                            </div>
                ...
                <Draggable key={File._id} draggableId={`File-${File._id}`} index={k}>
                    {(provided) => (
                        <div className="col-12 mFWS" key={File._id} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            ...
                            <img src={returnType(File.type)} alt="compnay" style={{ cursor: 'pointer' }} onClick={e => !sv && history.push(`/personal/myspace/user/${uId}/file/${File._id}`)} />
                            {!sv && <Link style={{ textDecoration: 'none', wordBreak: 'break-all' }} to={`/personal/myspace/user/${uId}/file/${File._id}`} className="f-n">{File.name.length > 35 ? `${File.name.substr(0, 35)}...` : File.name}</Link>}
