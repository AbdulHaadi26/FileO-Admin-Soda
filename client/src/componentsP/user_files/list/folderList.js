...
export default ({ list, uId, isList, ord, setDel, setLink, setMUpt, setCID, setCopy, setMove, disabled }) => {
    ...
    return isList ? listT.map((Cat, k) => <Droppable key={Cat._id} droppableId={`drop-Folder-${Cat._id}`}>
        {(provided) => (
            <div className="LI" {...provided.droppableProps} ref={provided.innerRef} key={Cat._id}>
                ...
                <Link style={{ textDecoration: 'none', marginLeft: '12px', wordBreak: 'break-all' }} to={`/personal/myspace/user/${uId}/files/${Cat._id}/list`} className="mr-auto">{Cat.name}</Link>
