...
import { clearUploadModal, getCatSelect, registerFileMod, registerFileNew, registerFileVer } from '../../../redux/actions/userFilesActions';
import '../style.css';
...
const Add = ({
    list, setting, registerFileMod, id, _id, onhandleModal, clearUploadModal, isUpt, profile, per, fileData, getCatSelect, registerFileNew, registerFileVer, fileD
}) => {
    const [name, setN] = useState(''), [errn, setErrN] = useState(false), [rList, setRL] = useState([]), [tempId, setTI] = useState(''), [size, setS] = useState(0), [fName, setFN] = useState(''),
        ...
        [errF, setErrF] = useState(false), [errB, setErrB] = useState(false), [errS, setErrS] = useState(false), [mimeT, setMT] = useState(''), [started, setStarted] = useState(0);
    ...
    const handleSubmit = e => {
        ...
        if (name && fS && !errB && mimeT) {
            ...
            let data = {
                _id: id, name: name, size: size, mime: mimeT, uploadType: 1,
                postedby: _id, org: id, category: tempId, type: type,
    ...
    const handleVer = (file, dFile) => {
        if (file) {
            ...
            let data = {
                ...
                active: true, type: typeS, description: '', fName: dFile.name, mod: true
            };
    ...
    return <Container profile={profile} num={16} isSuc={!isUpt} onSubmit={Submit} isUpt={isUpt} percent={per}>
        {started > 0 ? <UploadUserFile note={true} handleVer={handleVer} /> :
...
const mapStateToProps = state => {
    return {
        ...
        fileD: state.File.combinedFile
    }
}

export default connect(mapStateToProps, { registerFileMod, clearUploadModal, getCatSelect, registerFileVer, registerFileNew })(Add);
