...
const ModalBG = ({ onhandleModal, onhandleAdd, object }) => {
    ...
    return <Modal handleModal={onhandleModal} isOpt={true}>
        <h3 style={{ fontWeight: '600', fontSize: '18px', marginTop: '12px', marginBottom: '12px', padding: '6px 12px' }}>Edit Chore</h3>
        <hr />
        <div style={{ width: '100%', padding: '6px 12px' }}>
            <textarea type="text" placeholder="Type text here" style={{ height: '100px' }} className="comment" value={text} onChange={e => setText(e.target.value)} />
        </div>
        <hr />
