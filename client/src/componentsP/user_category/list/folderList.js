...
export default ({ list, uId, isList, ord, setDel, setLink, setMUpt, setCID, setSHMOD, setCopy, setMove, disabled }) => {
    ...
    return isList ? listT.map((Cat, k) => <Droppable key={Cat._id} droppableId={`drop-Folder-${Cat._id}`}>
        {(provided) => (
            <div className="LI" {...provided.droppableProps} ref={provided.innerRef} key={Cat._id}>
                ...
                <Link style={{ textDecoration: 'none', marginLeft: '12px', wordBreak: 'break-all' }} to={`/personal/myspace/user/${uId}/files/${Cat._id}/list`} className="mr-auto">{Cat.name} {Cat.updated && <span style={{ color: 'red', fontSize: '12px' }}>(Updated)</span>}</Link>
                ...
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
                    <h6 className="item-hover" style={{ position: 'relative', width: 'fit-content' }} onClick={e => setAct(active === k ? -1 : k)}>
                        ...
                        <div className="dropdown-content" style={{ display: `${active === k ? 'flex' : 'none'}`, top: '12px' }}>
                            <h6 className='s-l' style={bS} onClick={e => { setAct(-1); !disabled && setCID(Cat._id); }}>Upload File</h6>
                            <h6 className='s-l' style={bS} onClick={e => { setAct(-1); !disabled && setCopy(Cat) }}>Copy Folder</h6>
                            <h6 className='s-l' style={bS} onClick={e => { setAct(-1); setMove(Cat) }}>Move Folder</h6>
                            <h6 className='s-l' style={bS} onClick={e => { setAct(-1); setMUpt(Cat) }}>Edit Folder</h6>
                            <h6 className='s-l' style={bS} onClick={e => { setAct(-1); !disabled && setLink(Cat._id) }}>Generate Link</h6>
        ...
        {(provided) => (
            <div className="col-lg-2 col-4 mFWS" {...provided.droppableProps} ref={provided.innerRef}>
                ...
                <img src={Folder} alt="Folder" style={{ cursor: 'pointer' }} onClick={e => history.push(`/personal/myspace/user/${uId}/files/${Cat._id}/list`)} />
                <Link to={`/personal/myspace/user/${uId}/files/${Cat._id}/list`} className="f-n mr-auto" style={{ textDecoration: 'none', wordBreak: 'break-all' }}>{Cat.name.length > 35 ? `${Cat.name.substr(0, 35)}...` : Cat.name}</Link>
            </div>
        )}
    </Droppable>);
}
