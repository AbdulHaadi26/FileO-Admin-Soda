...
const ListCat = ({
    _id, string, handleS, tabNav, setTN, isList, fileData, type, handleT, fetchCombined, handleISL, getList, registerCat, updateCat, deleteCat, deleteFile,
    ...
    const [ord, setO] = useState(0), [active, setAct] = useState(false), [modalUpt, setMUpt] = useState(false), [ver, setVer] = useState(false), [verN, setVerN] = useState(false),
        ...
        }), [mD, setMDD] = useState(false), [sL, setSL] = useState([]), [list, setList] = useState([]),
        [listC, setListC] = useState(false),
        [sv, setSV] = useState(false), [activeV, setActV] = useState(false);
    ...
    return <DragDrop handleSuccess={handleSuccess}>
        <div className="col-11 c-d-w p-0">
                    ...
                    <DragDropContext onDragEnd={handleOnDragEnd}>
                        {listC && listC.length > 0 &&
                            <FolderList ord={ord} uId={_id} list={listC} isList={isList}
                                setMUpt={setMUpt} setLink={val => setSM(val)}
                            ...
                            />}
                        {list && list.length > 0 &&
                            <FileList list={list} isList={isList} handleSel={handleSel} sv={sv} ord={ord} uId={_id}
                                setEF={val => {
                                    setEF(true);
                                    getDetails(val);
                                }} setVerN={setVerN}
                                setF={setVer} setDel={setMDF} setLink={setSMF}
            ...
            {catId && <AddFile _id={_id} catId={catId} onhandleModal={e => setCID(false)} />}

            {ver && <AddVer uId={_id} verId={ver.versionId} onhandleModal={e => setVer(false)} />}
            {verN && <AddNew verId={verN.versionId} userId={_id} onhandleModal={e => setVerN(false)} />}

            {upt === 1 && <AddFile _id={_id} catId={''} onhandleModal={e => setUpt(false)} />}
            {upt === 2 && <AddFiles _id={_id} catId={''} onhandleModal={e => setUpt(false)} />}
