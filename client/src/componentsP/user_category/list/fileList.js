...
const List = ({
     uId, list, ord, sv, handleSel, sL, isList, downloadFile, setLink, setDel,
    setF, setEF, setVerN, setCopy, setMove, disabled
}) => {
                    ...
                    {(provided) => (
                        <div className="LI" style={{ borderRadius: '4px', position: 'relative' }} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            ...
                            <div className="mr-auto" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                <Link style={{ textDecoration: 'none', marginLeft: '12px', wordBreak: 'break-all' }} to={`/personal/myspace/user/${uId}/file/${File._id}`}>{File.name}</Link>
                            </div>
