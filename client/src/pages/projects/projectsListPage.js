...
const ListPage = ({ match, profile, isL, fetchProjectM, fetchProjectA }) => {
    ...
    return <Container profile={profile} num={13} isSuc={!isL && started > 0}>
        {profile.user.userType === 1 ? <PList tabNav={tabNav} setTN={setTN} orgName={profile.user.orgName} userId={profile.user._id} org={id} limit={limit} 
        limitMult={limitMult} string={string} handleS={onhandleS} handleLM={onhandleLM} handleL={onhandleL} getList={getList} isList={isList} setISL={setISL} /> :
