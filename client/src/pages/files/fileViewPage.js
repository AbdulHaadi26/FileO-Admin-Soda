...
const ViewPage = ({ match, getFile, profile, isSuc, file, isErr }) => {
    ...
    return <Container profile={profile} isSuc={isSuc && file} isErr={isErr} eT={'File Not Found'} num={9}>
        <ViewFile File={file} id={id} tabNav={tabNav} setTN={setTN} disabled={profile && profile.user && profile.user.current_employer && profile.user.current_employer.isDisabled} />
    </Container>
}
