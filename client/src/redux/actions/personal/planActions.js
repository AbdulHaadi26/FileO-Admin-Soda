...
export const registerPlan = data => async dispatch => {
    try {
        ...
        if (res.data.key && !res.data.error) {
            ...
            console.log(`/personal/myspace/user/${data._id}/plan/view/${res.data.key}`)
            history.push(`/personal/myspace/user/${data._id}/plan/view/${res.data.key}`);
...
export const deletePlan = data => async dispatch => {
    try {
        ...
        !data.skip && history.push(`/personal/myspace/user/${data.uId}/plan/list`)
    } catch {
        dispatch(logOut());
        dispatch(ModalProcess({ title: 'Session', text: 'Your session has expired. Please login again.', isErr: true }));
    }
