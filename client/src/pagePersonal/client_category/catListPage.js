...
import { fetchCombined } from '../../redux/actions/personal/clientCategoryAction';
import Container from '../container';
const List = lazy(() => import('../../componentsP/client_category/list'));

const ListPage = ({ profile, fetchCombined, isL, isLF, match }) => {
    const { _id } = match.params, [tabNav, setTN] = useState(0), [string, setS] = useState(''), [started, setStarted] = useState(0), [type, setT] = useState('All'), [isList, setISL] = useState(false);
    ...
    return <Container profile={profile} isSuc={!isL && !isLF && started > 0} num={17}>
        <List _id={_id} disabled={profile && profile.user && profile.user.current_employer && profile.user.current_employer.isDisabled}
