...
export default ({ list, id, isList, ord, auth, setMD, setCID, setMUpt }) => {
    ...
    return isList ? listT.map((Cat, k) => <div className="LI" key={Cat._id}>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end'  }}>
            <h6 className="item-hover" style={{ position: 'relative', width: 'fit-content', visibility: auth ? 'visible' : 'hidden' }} onClick={e => setAct(active === k ? -1 : k)}>
