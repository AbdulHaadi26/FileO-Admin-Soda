...
const Modal = ({
    ...
    const submitForm = e => {
        e.preventDefault();
        let data = { org: org, pkgId: pkg._id, difference: Number(active_plan.size - pkg.size) * 5 };
        if (pkg && pkg._id && active_plan._id !== pkg._id && active_plan.size > pkg.size) submitData(data, pkg);
