...
const Dashboard = ({ data, profile, catId, registerReq, registerPlan }) => {
    ...
    const { fileCount, fileList, projectCount, empReq, orgFileCount, docCount, otherCount, mediaCount, imageCount } = data;
    ...
    return <div style={{ width: '100%', display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
        <div className="col-lg-9 col-12 d-w">
            ...
            <div className="dsh p-0">
                {<Suspense fallback={<></>}><Dash org={current_employer && current_employer._id} or={current_employer} r={projectCount}
                    f={fileCount} _id={profile && profile._id} uStr={profile && profile.storageLimit} uptStr={profile && profile.storageUploaded}
                /></Suspense>}
            </div>
            ...
            {kI === 0 && <AddFile id={current_employer._id} _id={_id} catId={''} onhandleModal={e => setKI(false)} />}
