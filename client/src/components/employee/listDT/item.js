...
import Data from '../../../assets/exchange.svg';
...
const renderList = (list, id) => list.map(Emp => <div className="col-lg-2 col-4 mFWS" key={Emp._id}>
    <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
        <h6 className="item-hover" style={{ position: 'absolute', top: '12px', right: '8px', width: 'fit-content' }}>
            <div onClick={e => history.push(`/organization/${id}/data/transfer/employee/${Emp._id}`)} style={{
