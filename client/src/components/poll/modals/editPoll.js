...
export default ({ onhandleModal, onhandleAdd, Poll }) => {
    let dateMin = new Date(Date.now());
    dateMin.setDate(dateMin.getDate() + 1);
    dateMin = dateMin.toISOString().slice(0, 10);

    const [text, setText] = useState(Poll.name), [date, setDate] = useState(Poll.end_date), [description, setDescription] = useState(Poll.description),
        [p, setP] = useState(0), [questions, setQuestion] = useState(Poll.questions && Poll.questions.length > 0 ? Poll.questions : []), [item, setItem] = useState('');
    ...
    return <Modal handleModal={onhandleModal} isOpt={true}>
        <form onSubmit={e => onSubmit(e)}>
            <div style={{ width: '100%', display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center' }}>
                <h3 style={{ fontWeight: '600', fontSize: '18px', marginTop: '12px', marginBottom: '12px', padding: '6px 12px', marginRight: '12px' }}>Edit Poll</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', padding: '6px 12px', marginTop: '12px', marginBottom: '12px' }}>
                ...
                <button className="btn btn-primary" type="submit" style={{ marginLeft: '12px', fontSize: '14px', fontWeight: '600', padding: '6px 24px' }}>{p === 1 ? 'Update' : 'Next'}</button>
            </div>
        </form>
    </Modal>
};
