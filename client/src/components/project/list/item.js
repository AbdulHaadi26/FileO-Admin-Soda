...
export default ({ list, count, ord, onFetch, setTP, setDel, isList, setFA }) => {
    ...
    const renderList = () => isList ? listT.map((Item, k) => <div className="LI" key={Item._id}>
        <img src={listI[Item.icon ? Item.icon : 0].img} alt={listI[Item.icon ? Item.icon : 0].text} style={{ cursor: 'pointer', width: '30px', height: '30px' }}
            onClick={e => history.push(`/organization/${Item.org}/projects/${Item._id}/categories/list/page/0`)} />
        <Link style={{ textDecoration: 'none', wordBreak: 'break-all', fontSize: '14px', fontWeight: '400', marginLeft: '12px' }} className="mr-auto" to={`/organization/${Item.org}/projects/${Item._id}/categories/list/page/0`}>{Item.name}</Link>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end', marginLeft: 'auto' }}>
            <h6 className="item-hover" style={{ position: 'relative', width: 'fit-content' }} onClick={e => setAct(active === k ? -1 : k)}>
                ...
                <div className="dropdown-content" style={{ display: `${active === k ? 'flex' : 'none'}`, top: '12px' }}>
                    <h6 className='s-l' style={bS} onClick={e => { setAct(-1); setFA(Item._id); }}>View Participants</h6>
        ...
        <img src={listI[Item.icon ? Item.icon : 0].img} alt={listI[Item.icon ? Item.icon : 0].text} style={{ cursor: 'pointer' }}
            onClick={e => history.push(`/organization/${Item.org}/projects/${Item._id}/categories/list/page/0`)} />
        <Link style={{ textDecoration: 'none', wordBreak: 'break-all', fontSize: '14px', fontWeight: '400', marginTop: '12px', marginLeft: 'auto', marginRight: 'auto' }} to={`/organization/${Item.org}/projects/${Item._id}/categories/list/page/0`}>{Item.name}</Link>
    </div>);

    return <>
        {renderList(list)}
        {count > 12 && <div className="pLW col-12 p-0">
