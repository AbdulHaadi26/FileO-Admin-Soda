...
const List = ({ onFetch, list, count, pId, org, assignEmployee, getList }) => {

    const handleAdd = async user => {
            let data = { userId: user ? user._id : '', pId, org: org, name: user.name, orgName: user.orgName };
            await assignEmployee(data);
            getList();
    };
...
export default connect(null, { assignEmployee })(List);