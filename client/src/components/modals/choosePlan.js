...
import { billingReminder, getPackages, generateTrialOrder } from '../../redux/actions/organizationActions';
...
const ModalB = ({ billingReminder, packages, getPackages, ModalProcess, onNext, generateTrialOrder }) => {
    ...
    const submitForm = async () => {
        let data = { pkgId: pkg._id, price: (Number(pkg ? pkg.price : 0)) + (3 * 275) };
        if (pkg && pkg._id && pkg.size > 25) {
    ...
    return <Modal handleModal={onhandleModal} isOpt={true}>
        <form onSubmit={e => { e.preventDefault(); submitForm(); }}>
            ...
            <div className="col-12" style={{ padding: '6px 24px' }}>
                ...
                <h6 style={tS}>User (minimum 3)</h6>
                <div className="input-group" style={iG}>
                    <input type={'number'} disabled className="form-control" placeholder={'Enter package'} value={3} />
                </div>
                <h6 style={tS}>PRICE</h6>
                <h6 style={{ fontSize: '16px', color: 'green', fontWeight: '500', width: '90%' }}>
                    PKR {(Number(pkg ? pkg.price : 0)) + (3 * 275)}
                </h6>
            </div>
            <hr />
...
export default connect(mapStateToProps, { billingReminder, getPackages, ModalProcess, generateTrialOrder })(ModalB);
