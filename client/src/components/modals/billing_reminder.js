...
const ModalB = ({ billingReminder, Organization, type }) => {
    ...
    if (Organization && Organization.org && Organization.org.trail_end_date && Organization.org.isTrail) {
        let date1 = new Date(Organization.org.trail_end_date);
    ...
    return ((type < 2 && daysLeft <= 0) || type === 2) ? <Modal handleModal={onhandleModal} isOpt={true}>
        <form onSubmit={e => {
            e.preventDefault();
            billingReminder(2)
        }}>
            ...
            <div className="col-12" style={{ padding: '6px 24px', display: 'flex', flexDirection: 'row', marginTop: '24px', alignItems: 'center' }}>
                ...
                <div className="col-8" style={{ display: 'flex', flexDirection: 'column' }}>
                    {type === 2 && date && <h6 style={{ fontSize: '16px', fontWeight: '600' }}>Your Free Trial period of File-O will end on {date.toUTCString().substr(0, 17)}.</h6>}
                    {type < 2 && date && <h6 style={{ fontSize: '16px', fontWeight: '600' }}>Your Free Trial period of File-O has ended on {date.toUTCString().substr(0, 17)}.</h6>}
                    {type === 2 && <h6 style={{ fontSize: '14px', fontWeight: '400', marginTop: '12px' }}>To continue using File-O please use a subscription plan.</h6>}
                    {type < 2 && <h6 style={{ fontSize: '14px', fontWeight: '400', marginTop: '12px' }}>You will not be able to use funtionality of the application. To continue using File-O please use a subscription plan or contact your File-O administrator.</h6>}
                    {type === 2 && <h6 style={{ fontSize: '11px', fontWeight: '400', marginTop: '12px' }}><b>Note:</b> Your free trial period won't be effected and we won't charge you until after the free trial expires.</h6>}
                </div>
            </div>
...
export default connect(mapStateToProps, { billingReminder })(ModalB);
