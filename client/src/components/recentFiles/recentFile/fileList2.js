...
export default ({ list, id, ord, isList }) => {
    ...
    const renderList = () => listT.map((File, k) => isList ? <div className="LI" key={File._id} style={{alignItems:'flex-start'}}>
        ...
        <div style={{ flexGrow: 1, display: 'flex', marginLeft: '12px', flexDirection: 'column' }}>
            <Link className="f-n" style={{ textDecoration: 'none', wordBreak: 'break-all', fontSize: '14px' }} to={`${getBaseUrl(File, File.renderType)}${File._id}`}>{File.name}</Link>
            <h6 style={{ fontSize: '12px', marginTop: '8px' }}>{File.renderType}</h6>
            <h6 style={c}>{renderDate(File.date)}</h6>
        </div>
            ...
            <Link className="f-n" to={`${getBaseUrl(File, File.renderType)}${File._id}`} style={{ textDecoration: 'none', wordBreak: 'break-all' }}>{File.name.length > 35 ? `${File.name.substr(0, 35)}...` : File.name}</Link>
            <h6 style={{ fontSize: '12px', marginTop: '8px' }}>{File.renderType}</h6>
            <h6 style={c}>{renderDate(File.date)}</h6>
        </div>);
