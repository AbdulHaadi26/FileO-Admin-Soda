...
const List = ({ id, _id, list, count, ord, onFetch, isList }) => {
    ...
    const renderList = () => listT.map(Note => <div className="col-lg-3 col-12 mFW p-0" key={Note._id}>
            <div className="col-lg-11 col-12 fIW">
                <h6 className="f-n">{Note.noteTitle}</h6>
                {Note.updated && <h6 className="updated">Updated</h6>}
                <h6 className="f-r">{Note.sharedByName}</h6>
                ...
                <Link className="overlay" to={`/organization/${id}/myspace/user/${_id}/notes/view/${Note.noteId._id}`}>
                    <span className="text">
