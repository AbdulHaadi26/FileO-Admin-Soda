...
import { fetchCatModal, moveCat, copyCat } from '../../../../redux/actions/project_categoryActions';
import Loader from '../../../loader/simpleLoader';
import ModalBg from '../../../containers/modalBgContainer';
import { cutFile, copyFile } from '../../../../redux/actions/project_filesActions';
...
const Modal = ({
    ...
    setMA, type, moveCat, cutFile, copyFile, copyCat, pId, name,
    callFunc
}) => {

    useEffect(() => {
        fetchCatModal({ catId: data.sId, _id: data._id, pId });
    }, [fetchCatModal, data.sId, data._id, pId]);
    ...
    const BreadCrumb = () => {
        if (catData && catData.cat) {
            return <>
                <h6 onClick={e => setSId('')} style={{ fontWeight: '600', fontSize: '12px', marginRight: '3px', cursor: 'pointer', color: '#0000EE' }}>{name}</h6>
        ...
        return <h6 onClick={e => setSId('')} style={{ fontWeight: '600', fontSize: '12px', marginRight: '6px', cursor: 'pointer', color: '#0000EE' }}>{name}</h6>
    };
    ...
    return <ModalBg handleModal={onhandleModal}>
        ...
        <div className="col-12" style={{ padding: '6px 12px' }}>
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'row', flexWrap: 'wrap', marginTop: '12px' }}>
                {catData && catData.catList && catData.catList.length > 0 ? <Suspense fallback={<></>}> <FolderList list={catData.catList} _id={data._id} setCId={setCId} cId={data.catId} setSId={setSId} /> </Suspense> :
                    (!catData || !catData.catList) ? <div style={lS}><Loader /></div> : <h6 className="f-n" style={eS}>No folder found</h6>}
            </div>
        </div>
        <hr />
