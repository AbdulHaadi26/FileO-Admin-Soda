...
import { fetchCombinedL } from '../../redux/actions/userCategoryActions';
...
const SearchBar = ({ fetchCombinedL, isL, fileData, org, pId }) => {
    ...
    const renderFileList = (list) => {
        ...
        return listF.map((File, k) => <div className="LI" style={{ borderRadius: '4px', position: 'relative', border:'none', cursor:'pointer' }} key={k} onClick={e => history.push(`/organization/${File.org}/myspace/user/${File.postedby}/file/${File._id}`)}>
    ...
    const renderCatList = (list) => {
        let listF = list.filter((i, k) => k < 7);
        return listF.map((File, k) => <div className="LI" style={{ borderRadius: '4px', position: 'relative', border:'none', cursor:'pointer' }} key={k} onClick={e => history.push(`/organization/${File.org}/myspace/user/${File.uId}/files/${File._id}/list`)}>
        ...
        <div className="searchbox" style={{ width: '100%', position: 'relative' }}>
            ...
            <div className="dropdown-content-sea" ref={node} style={{ display: `${active ? 'flex' : 'none'}` }}>
                ...
                {!isL && fileData && fileData.cats && fileData.files && fileData.cats.length + fileData.files.length >= 15 &&
                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', margin: 'auto' }}>
                        <Link to={`/organization/${org}/myspace/user/${pId}/category/data/${value}/list`} style={{ fontSize: '14px', margin: '12px 0px' }}>Show more</Link>
                    </div>}
            </div>
        </div>
    </div>
