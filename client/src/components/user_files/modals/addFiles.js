...
import { registerFileM } from '../../../redux/actions/userFilesActions';
...
const Add = ({ catId, setting, registerFileM, id, _id, onhandleModal }) => {
        ...
        if (e.target.files && e.target.files.length > 0 && e.target.files.length < 11) {
            ...
            for (let i = 0; i < e.target.files.length; i = i + 1) {
                ...
                if (file.size <= (fileSize * 1024 * 1024)) {
                    let data = { name: file.name, size: file.size, mime: file.type, category: catId, type: finalizeType(type[type.length - 1]), org: id, postedby: _id, description: '', fName: file.name }
                    ar.push(data);
                    fL.push(file);
                }
            }
