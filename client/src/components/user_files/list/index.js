...
const List = ({
    fetchCombined, id, _id, catId, fileData, getList, string, type, getFileDetailsM,
    ...
    const [ord, setO] = useState(0), [sv, setSV] = useState(false), [name, setName] = useState(''), [sMF, setSMF] = useState(false), [cId, setCID] = useState(false), [data, setData] = useState(false),
        [active, setAct] = useState(false), [sL, setSL] = useState([]), [list, setList] = useState([]), [mD, setMD] = useState(false),
    ...
    const handleSuccess = (file) => {
        if (file && !disabled) {
            ...
            let data = {
                ...
                org: id, category: catId, type: typeS,
    ...
    const BreadCrumb = () => {
        if (category && category.cat) {
            return <>
                <Link to={`/organization/${id}/myspace/user/${_id}/category/list`} style={{ fontWeight: '600', fontSize: '12px', marginRight: '3px', cursor: 'pointer', marginTop: '-2px' }}>My Space</Link>
                <Link to={`/organization/${id}/myspace/user/${_id}/category/list`} style={{ fontWeight: '600', fontSize: '12px', marginRight: '3px', cursor: 'pointer', marginTop: '-2px' }}>{'>'}</Link>
                {category.cat.pCat && category.cat.pCat.length > 0 && category.cat.pCat.map((pCat, k) => <div style={{ display: 'flex', flexDirection: 'row' }} key={k}>
                    <Link to={`/organization/${id}/myspace/user/${_id}/files/${pCat._id}/list`}
                    ...
                    <h6 className={`mTHS`} style={{ padding: '10px 8px 6px 8px', position: 'relative', marginTop: '0px', marginBottom: '0px', marginRight: '-3px' }}>
                        ...
                        <div className="dropdown-content-c" ref={node} style={{ display: `${active ? 'flex' : 'none'}` }}>
                            {!sv ? <>
                                ...
                                <h6 className='s-l' onClick={e => { setSHMODC(catId); setAct(false); }}>Share Folder</h6>
                ...
                <DragDropContext onDragEnd={handleOnDragEnd}>
                    ...
                    {list && list.length > 0 &&
                        <FileList sL={sL} setDel={setMDFL} setLink={setSMFL} setF={setVer} setEF={val => { setEF(val); getDetails(val) }} setVerN={setVerN}
                            isList={isList} handleSel={handleSel} list={list} ord={ord} sv={sv} id={id} catId={catId} uId={_id} setSHMOD={setSHMOD}
