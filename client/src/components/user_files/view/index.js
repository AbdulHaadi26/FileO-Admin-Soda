...
const View = ({ File, AddToF, DelToF, tabNav, setTN, discussion, updated, profile, count, offset, setOF, addComment, updateChat, downloadFile, generateUrl, disabled }) => {
    const [width, setWidth] = useState(0), [cmp, setCmp] = useState(false), [version, setVer1] = useState([]), [version2, setVer2] = useState([]), [url1, setUrl1] = useState(''), [id1, setId1] = useState(''), [mId, setMID] = useState(''),
        ...
        [d2, setD2] = useState(''), [desc1, setDesc1] = useState(''), [desc2, setDesc2] = useState(''), [i1, setI1] = useState(''), [message, setMessage] = useState(''), [active, setAct] = useState(false),
        [i2, setI2] = useState(''), [t1, setT1] = useState(''), [t2, setT2] = useState(''), [i, setI] = useState(0), [shMod, setSHMOD] = useState(false);
    const node = useRef({});
    ...
    if (File && File.file) var { name, url, _id, postedby, org, category, versionId } = File.file;
...
export default connect(null, { AddToF, DelToF, addComment, downloadFile, generateUrl })(View);