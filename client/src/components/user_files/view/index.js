...
const View = ({ File, getF, AddToF, DelToF, tabNav, setTN, discussion, updated, profile, count, offset, setOF, addComment, updateChat }) => {
    const [width, setWidth] = useState(0), [cmp, setCmp] = useState(false), [version, setVer1] = useState([]), [version2, setVer2] = useState([]), [url1, setUrl1] = useState(''), [id1, setId1] = useState(''), [mId, setMID] = useState(''),
        ...
        [d2, setD2] = useState(''), [desc1, setDesc1] = useState(''), [desc2, setDesc2] = useState(''), [i1, setI1] = useState(''), [message, setMessage] = useState(''),
        [i2, setI2] = useState(''), [t1, setT1] = useState(''), [t2, setT2] = useState(''), [i, setI] = useState(0), [string, setS] = useState(''), [limitMult, setLM] = useState(0), [limit, setL] = useState(12), [limitMult2, setLM2] = useState(0),
    ...
    if (File && File.file) var { name, url, _id, postedby, org, category } = File.file;
    ...
    return <div className="col-11 p-0 f-w">
        ...
        <div style={dF}>
            ...
            {id1 !== '' && !cmp && <ButtonDown id={id1} showShareModal={e => setSHMOD(true)} showModal={handleModal} showEmailModal={handleEmailModal} />}
        ...
        {shMod && <Assigned id={org} _id={postedby} fId={_id} limit={limit} limitMult={limitMult} string={string} limit2={limit2} limitMult2={limitMult2} string2={string2} tabNav={tabNav}
