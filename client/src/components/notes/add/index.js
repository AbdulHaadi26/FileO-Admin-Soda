...
import '../snow.css';
import { registerNote, deleteRec } from '../../../redux/actions/noteActions';
...
const Add = ({ org, _id, registerNote, setting, deleteRec }) => {
    ...
    const handleSubmit = e => {
        ...
        if (c && v && (note ? (((new TextEncoder().encode(note)).length) / (1024 * 1024) < 98) : true)) {
            ...
            let data = {
                ...
                editable: cbEditable, catList: cats
            };
            registerNote(data);
        } else {
            !v && setErrV(true);
...
export default connect(mapStateToProps, { registerNote, deleteRec })(Add);