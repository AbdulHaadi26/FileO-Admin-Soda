...
const Add = ({ org, _id, registerNote, setting, deleteRec, clearNote, noteS, profile }) => {
    const [note, setNote] = useState(''), [v, setV] = useState(''), [at, setAT] = useState([]), [cId, setCId] = useState(''),
        [errV, setErrV] = useState(false), [f, setF] = useState(''), [ModalF, setMF] = useState(''), [MC, setMC] = useState(false),
        [MCV, setMCV] = useState(false), [ct, setCT] = useState([]), [lEx, setLEX] = useState(false),
        [fd, setFD] = useState(''), [t, setT] = useState(2), [cbEditable, setCBE] = useState(false), [modal, setModal] = useState(false),
        [MF, setMDF] = useState(false), [icon, setIcon] = useState(0);
    ...
    const handleSubmit = e => {
        ...
        if (v && (note ? (((new TextEncoder().encode(note)).length) / (1024 * 1024) < 98) : true)) {
            ...
            let data = {
                ...
                editable: cbEditable, catList: cats, icon
            };
            registerNote(data);
        } else {
            !v && setErrV(true);
    ...
    return <RegisterC onSubmit={handleSubmit} text={'Add Note'}>
        <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', position: 'relative' }}>
            <button className="btn btn-primary btn-send" type="submit">Add Note</button>
        </div>
        <Suspense fallback={<></>}> <InputText t={'TITLE'} plh={`Enter note title`} tp={'text'} val={v} handleInput={handleInput} err={errV} /> </Suspense>
        <h6 style={iS}>NOTE</h6>
        ...
        <h6 style={iS}>ICON</h6>
        <Icons i={icon} setI={setIcon} isTask={false} />

        {noteS && <Assigned id={org} isShow={true} isTask={false} _id={_id} nId={noteS} onhandleModal={e => {
            clearNote();
            history.push(`/organization/${org}/myspace/user/${_id}/notes/list/page/0`);
        }} />}
        ...
        {MC && <Suspense fallback={<></>}> <ModalRec isTask={false} noteName={v} _id={_id} org={org} onhandleModal={handleModalC} fileL={setting && setting.setting && setting.setting.maxFileSize ? setting.setting.maxFileSize : 5} onSubmit={handleRec} /> </Suspense>}
        {MCV && <Suspense fallback={<></>}> <ModalRecV isTask={false} noteName={v} _id={_id} org={org} onhandleModal={handleModalCV} fileL={setting && setting.setting && setting.setting.maxFileSize ? setting.setting.maxFileSize : 5} onSubmit={handleRec} /> </Suspense>}
    </RegisterC>
}
...
export default connect(mapStateToProps, { registerNote, deleteRec, clearNote })(Add);