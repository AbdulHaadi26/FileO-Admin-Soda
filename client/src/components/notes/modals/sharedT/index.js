...
import { fetchAssigned, fetchAssignedSearch, fetchEmp, fetchEmpSearch, deleteAssignedAll } from '../../../../redux/actions/sharedTasksActions';
...
const List = ({
    ...
    return <Modal handleModal={onhandleModal}>
        <h3 style={{ fontWeight: '600', fontSize: '18px', marginTop: '12px', marginBottom: '12px', padding: '6px 12px' }}>Share {isTask ? 'Task' : 'Note'}</h3>
        <hr />
        <div className="col-12" style={{ padding: '6px 12px' }}>
            {type !== 'Users' && <button style={{ marginBottom: '12px', marginLeft: 'auto', marginTop: '0px' }} className="btn btn-dark" onClick={e => deleteAssignedAll({ noteId: nId })}>Unshare With All</button>}
