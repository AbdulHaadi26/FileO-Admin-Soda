...
import { transferOwnership } from '../../../../redux/actions/sharedTasksActions';
import Minus from '../../../../assets/exchange.svg';
import { connect } from 'react-redux';
import User from '../../../../assets/static/user.png';

const List = ({ list, count, nId, transferOwnership, onFetch, onhandleModal }) => {

    const handleDelete = async (e, user) => {
        
        let data = { userId: user ? user._id : '', noteId: nId };
    ...
    const renderListF = () => list.map(i => i.sharedWith && <div className="col-lg-2 col-4 mFWS" key={i._id}>
        <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
            <h6 className="item-hover" style={{ position: 'absolute', top: '12px', right: '8px', width: 'fit-content' }}>
                <div onClick={e => handleDelete(e, i.sharedWith ? i.sharedWith : '')} style={{
...
export default connect(null, { transferOwnership })(List);